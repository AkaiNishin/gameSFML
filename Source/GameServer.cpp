...
GameServer::GameServer(sf::Vector2f battlefieldSize)
    : mThread(&GameServer::executionThread, this), mListeningState(false),
      mClientTimeoutTime(sf::seconds(3.f)), mMaxConnectedPlayers(10),
      mConnectedPlayers(0), mWorldHeight(5000.f),
      mBattleFieldRect(0.f, mWorldHeight - battlefieldSize.y, battlefieldSize.x,
                       battlefieldSize.y),
      mBattleFieldScrollSpeed(-50.f), mAircraftCount(0), mPeers(1),
      mAircraftIdentifierCounter(1), mWaitingThreadEnd(false),
      mLastSpawnTime(sf::Time::Zero), mTimeForNextSpawn(sf::seconds(5.f)) {
...
void GameServer::executionThread() {
  ...
  while (!mWaitingThreadEnd) {
    ...
    while (stepTime >= stepInterval) {
      mBattleFieldRect.top +=
          mBattleFieldScrollSpeed * stepInterval.asSeconds();
...
void GameServer::tick() {
  ...
  // Remove IDs of aircraft that have been destroyed (relevant if a client has
  // two, and loses one)
    ...
    if (mBattleFieldRect.top > 600.f) {
      ...
      for (std::size_t i = 0; i < enemyCount; ++i) {
        ...
        packet << static_cast<sf::Int32>(1 +
                                         randomInt(Aircraft::TypeCount - 1));
...
void GameServer::updateClientState() {
  ...
  updateClientStatePacket << static_cast<float>(mBattleFieldRect.top +
                                                mBattleFieldRect.height);
  updateClientStatePacket << static_cast<sf::Int32>(mAircraftInfo.size());

  FOREACH(auto aircraft, mAircraftInfo)
  updateClientStatePacket << aircraft.first << aircraft.second.position.x
                          << aircraft.second.position.y;
...
void GameServer::handleIncomingConnections() {
  ...
  if (mListenerSocket.accept(mPeers[mConnectedPlayers]->socket) ==
    ...
    mPeers[mConnectedPlayers]->lastPacketTime =
        now(); // prevent initial timeouts
    mAircraftCount++;
    mConnectedPlayers++;
  ...
  for (auto itr = mPeers.begin(); itr != mPeers.end();) {
    if ((*itr)->timedOut) {
      ...
      FOREACH(sf::Int32 identifier, (*itr)->aircraftIdentifiers) {
        sendToAll(sf::Packet()
                  << static_cast<sf::Int32>(Server::PlayerDisconnect)
                  << identifier);
...
void GameServer::informWorldState(sf::TcpSocket &socket) {
  ...
  for (std::size_t i = 0; i < mConnectedPlayers; ++i) {
    if (mPeers[i]->ready) {
      ...
      packet << identifier << mAircraftInfo[identifier].position.x
