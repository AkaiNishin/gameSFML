...
Aircraft::Aircraft(Type type, const TextureHolder &textures,
    ...
    : Entity(Table[type].hitpoints), mType(type),
      ...
      mExplosion(textures.get(Textures::Explosion)), mFireCommand(),
      mMissileCommand(), mFireCountdown(sf::Time::Zero), mIsFiring(false),
      mIsLaunchingMissile(false), mShowExplosion(true), mExplosionBegan(false),
      mSpawnedPickup(false), mPickupsEnabled(true), mFireRateLevel(1),
      mSpreadLevel(1), mMissileAmmo(2), mDropPickupCommand(),
      mTravelledDistance(0.f), mDirectionIndex(0), mMissileDisplay(nullptr),
      mIdentifier(0) {
...
int Aircraft::getIdentifier() { return mIdentifier; }
...
void Aircraft::checkPickupDrop(CommandQueue &commands) {
  // Drop pickup, if enemy airplane, with probability 1/3, if pickup not yet
  // dropped and if not in network mode (where pickups are dropped via packets)
...
void Aircraft::checkProjectileLaunch(sf::Time dt, CommandQueue &commands) {
  ...
  if (mIsFiring && mFireCountdown <= sf::Time::Zero) {
    ...
    playLocalSound(commands, isAllied() ? SoundEffect::AlliedGunfire
