...
ParticleNode::ParticleNode(Particle::Type type, const TextureHolder &textures)
    : SceneNode(), mParticles(), mTexture(textures.get(Textures::Particle)),
      mType(type), mVertexArray(sf::Quads), mNeedsVertexUpdate(true) {}
...
void ParticleNode::addVertex(float worldX, float worldY, float texCoordX,
...
void ParticleNode::computeVertices() const {
  ...
  FOREACH(const Particle &particle, mParticles) {
    ...
    float ratio =
        particle.lifetime.asSeconds() / Table[mType].lifetime.asSeconds();
