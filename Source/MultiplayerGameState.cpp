...
MultiplayerGameState::MultiplayerGameState(StateStack &stack, Context context,
    ...
    : State(stack, context),
      ...
      mWindow(*context.window), mTextureHolder(*context.textures),
      mConnected(false), mGameServer(nullptr), mActiveState(true),
      mHasFocus(true), mHost(isHost), mGameStarted(false),
      mClientTimeout(sf::seconds(2.f)), mTimeSinceLastPacket(sf::seconds(0.f)) {
  ...
  // We reuse this text for "Attempt to connect" and "Failed to connect"
  // messages
...
void MultiplayerGameState::draw() {
  if (mConnected) {
    ...
    if (mLocalPlayerIdentifiers.size() < 2 &&
        mPlayerInvitationTime < sf::seconds(0.5f))
...
bool MultiplayerGameState::update(sf::Time dt) {
  ...
  if (mConnected) {
    ...
    for (auto itr = mPlayers.begin(); itr != mPlayers.end();) {
      ...
      if (std::find(mLocalPlayerIdentifiers.begin(),
      ...
      FOREACH(sf::Int32 identifier, mLocalPlayerIdentifiers) {
        if (Aircraft *aircraft = mWorld.getAircraft(identifier))
          positionUpdatePacket
              << identifier << aircraft->getPosition().x
...
void MultiplayerGameState::handlePacket(sf::Int32 packetType,
                                        sf::Packet &packet) {
  switch (packetType) {
  ...
  case Server::PlayerEvent: {
    ...
    if (itr != mPlayers.end())
      itr->second->handleNetworkEvent(static_cast<Player::Action>(action),
  ...
  case Server::UpdateClientState: {
    ...
    float currentViewPosition =
        mWorld.getViewBounds().top + mWorld.getViewBounds().height;

    // Set the world's scroll compensation according to whether the view is
    // behind or too advanced
    mWorld.setWorldScrollCompensation(currentViewPosition /
    ...
    for (sf::Int32 i = 0; i < aircraftCount; ++i) {
      ...
      bool isLocalPlane =
          std::find(mLocalPlayerIdentifiers.begin(),
                    mLocalPlayerIdentifiers.end(),
